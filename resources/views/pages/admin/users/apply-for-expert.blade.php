...
@php
    ...
    $formState = [
        ...
        'categoryId' => (string) old('category_id', ''),
        'subcategoryId' => (string) old('subcategory_id', ''),
        'skillIds' => array_map('strval', old('skill_ids', [])),
        'introVideoType' => old('intro_video_type', 'none'),
        'education' => old('education', $defaultEducation),
        'experience' => old('experience', $defaultExperience),
        'portfolio' => old('portfolio', $defaultPortfolio),
...
@section('content')
    <div x-data="{
        ...
        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            <x-common.page-breadcrumb pageTitle="Apply for expert" />
            <a href="{{ route('admin.users.index') }}" class="text-sm font-medium text-brand-500 hover:text-brand-600">
                ← Back to users
            </a>
        </div>
        ...
        <div class="mb-6">
            <p class="text-sm text-gray-500 dark:text-gray-400">
                Fill expert application details for
                <span class="font-medium text-gray-800 dark:text-white/90">{{ $user->name }}</span>
                ({{ $user->email }}). On submit, the profile is created and the user becomes an expert.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.users.apply-for-expert.store', $user) }}"
            ...
            <x-common.component-card title="Profile">
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" name="professional_headline" value="{{ old('professional_headline') }}"
                            required class="{{ $input }}">
                    </div>
                    <div>
                        <label class="{{ $label }}">Bio <span class="text-red-500">*</span></label>
                        <textarea name="bio" required class="{{ $textarea }}">{{ old('bio') }}</textarea>
                    </div>
                    <div class="grid grid-cols-1 gap-5 lg:grid-cols-2">
                        <div>
                            ...
                            <input type="number" min="0" max="80" name="years_of_experience"
                                value="{{ old('years_of_experience') }}" required class="{{ $input }}">
                        </div>
                        <div>
                            <label class="{{ $label }}">Registration value <span class="text-red-500">*</span></label>
                            <input type="text" name="registration_value" value="{{ old('registration_value') }}"
                                required class="{{ $input }}">
                        </div>
                    </div>
                    <div>
                        ...
                        <input type="text" name="languages"
                            value="{{ is_array(old('languages')) ? implode(', ', old('languages')) : old('languages', 'en, bn') }}"
            ...
            <x-common.component-card title="Intro video">
                ...
                <div x-show="introVideoType === 'url'">
                    ...
                    <input type="url" name="intro_video_url" value="{{ old('intro_video_url') }}"
