...
@section('content')
    <div x-data='{
        ...
        editAction: @json($editHasErrors && old('subcategory_id') ? route('taxonomy.subcategories.update', old('subcategory_id')) : ''),
        openEdit(btn) {
            this.editAction = btn.dataset.updateUrl;
            this.$refs.editParentCategory.value = btn.dataset.parentCategoryId;
            ...
            this.$refs.editSubcategoryId.value = btn.dataset.subcategoryId;
            this.editOpen = true;
        },
        async confirmDelete(e, name) {
            ...
            const { isConfirmed } = await window.Swal.fire({
                ...
                text: name
                    ...
                    : "This subcategory will be removed permanently.",
                icon: "warning",
        ...
        <x-common.page-breadcrumb pageTitle="Subcategories" />

        <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
            <p class="text-sm text-gray-500 dark:text-gray-400">Subcategories belong to one category each.</p>
                            ...
                            <tr>
                                ...
                                <td class="px-5 py-4 text-right text-sm">
                                    <button type="button"
                                        ...
                                        data-update-url="{{ route('taxonomy.subcategories.update', $subcategory) }}"
                                        data-subcategory-id="{{ $subcategory->id }}"
                                        data-parent-category-id="{{ $subcategory->category_id }}"
                                    ...
                                    <form action="{{ route('taxonomy.subcategories.destroy', $subcategory) }}" method="post"
                                        class="ml-3 inline-block">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button"
                                            class="text-red-600 hover:text-red-700 dark:text-red-400"
                                            @click="confirmDelete($event, @js($subcategory->name))">
                                            Delete
                                        </button>
                                    </form>
                                </td>
        ...
        <div x-show="createOpen" x-cloak
            ...
            <div @click.stop
                ...
                <h3 class="mb-6 text-lg font-medium text-gray-800 dark:text-white/90">New subcategory</h3>
                <form action="{{ route('taxonomy.subcategories.store') }}" method="post" class="space-y-5">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="create_category_id" name="category_id" required
                                class="{{ $select }} @error('category_id') border-red-500 @enderror">
                                <option value="">Select category</option>
                                @foreach ($parentCategories as $cat)
                                    <option value="{{ $cat->id }}"
                                        @selected(old('_form') === 'create' && (string) old('category_id') === (string) $cat->id)>
                                        {{ $cat->name }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div x-show="editOpen" x-cloak
            ...
            <div @click.stop
                ...
                <h3 class="mb-6 text-lg font-medium text-gray-800 dark:text-white/90">Edit subcategory</h3>
                <form method="post" :action="editAction" class="space-y-5">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="_form" value="edit">
                    <input type="hidden" name="subcategory_id" x-ref="editSubcategoryId"
                        value="{{ old('_form') === 'edit' ? old('subcategory_id') : '' }}">

                    <div>
                        <label for="edit_category_id" class="{{ $label }}">Category <span
                                class="text-red-500">*</span></label>
                        <div class="relative">
                            <select id="edit_category_id" name="category_id" required x-ref="editParentCategory"
                                class="{{ $select }} @error('category_id') border-red-500 @enderror">
                                <option value="">Select category</option>
                                @foreach ($parentCategories as $cat)
                                    <option value="{{ $cat->id }}"
                                        @if ($editHasErrors && (string) old('category_id') === (string) $cat->id) selected @endif>
                                        {{ $cat->name }}
                                    </option>
                                @endforeach
                            </select>
